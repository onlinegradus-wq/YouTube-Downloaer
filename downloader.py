...
def _download_media_sync(url: str, mode: str = "video", quality: str = "720") -> Tuple[Optional[str], str, str]:
    """Videoni yoki audioni 100% muammosiz yuklab olish (yt-dlp format 18/140 + pytubefix fallback)."""
    outtmpl = os.path.join(DOWNLOAD_DIR, '%(id)s_%(ext)s.%(ext)s')

    if mode == "audio":
        fmt = '140/ba/bestaudio/best'
    else:
        h = quality if quality.isdigit() else "720"
        fmt = f'best[height<={h}][filesize<=50M]/18/b/best[filesize<=50M]/best'
    ...
    # 1-urinish: yt-dlp ultra-fast format 18/140 (O'ta ishonchli universal YouTube formati)
    try:
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            info = ydl.extract_info(url, download=True)
            if info:
        ...
        print(f"[WARN] yt-dlp format 18/140 download failed: {e}")

    # 2-urinish: pytubefix multi-client fallback
    ...
    for c in clients:
        try:
            ...
            if mode == "audio":
                audio_stream = yt.streams.filter(only_audio=True).first() or yt.streams.get_audio_only()
                if audio_stream:
                    target_file = audio_stream.download(output_path=DOWNLOAD_DIR, filename=f"{video_id}.mp3")
                    return target_file, title, "SUCCESS"
            else:
                stream = (
                    yt.streams.filter(progressive=True, file_extension='mp4').first() or
                    yt.streams.filter(progressive=True).first() or
                    yt.streams.first()
                )
                if stream:
